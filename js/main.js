...
(function () {
    ...
    $('.language-skill').bind('inview', function (event, visible, visiblePartX, visiblePartY) {
        if (visible) {
            $('.chart').easyPieChart({
                easing: 'easeOut', delay: 3000, scaleColor: false, animate: 2000, onStep: function (from, to, percent) { this.el.children[0].innerHTML = Math.round(percent); }
            });
        }
    });
}());
...
function Scroll() {
    ...
    $.each(contentTop, function (i) {
        if (winTop > contentTop[i] - rangeTop) {
            $('.navbar-nav li.scroll').removeClass('current').eq(i).addClass('current');
        }
    })
};
(function () {
    function menuToggle() {
        ...
        if (windowWidth > 991) {
            $(window).on('scroll', function () {
                ...
                else { $('.home-two .navbar').removeClass('fixed-top animated fadeInDown'); };
            }
            );
        } else { $('.home-two .navbar').addClass('fixed-top'); };
    }
    menuToggle();
}());
